...
function lifecyclePolicyBlock(autonomous: boolean): string {
 return [
  ...
  '- update_goal({status: "blocked"}) only after the SAME blocker recurs on three consecutive goal turns; keep trying concrete steps before then. A blocker only the user can clear (installing a tool, credentials, a decision) is blocked immediately, never a wait: new waits declare depends_on and only an external producer qualifies.',
